...
export default () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            setDept(res.results)
            //20260308 wade chen 由於從subpayload帶入的department物件沒有id，只有name，所以這邊要轉換一下
            const deptID = (res.results.find((d: Department) => d.department_name === subpayload.department?.department_name)?.id)
            if (deptID) {
                setDeptValue(deptID)
                setSubpayload({
                    ...subpayload, department: {
                        id: deptID,
                        department_name: subpayload.department?.department_name
                    }
                })
            }
        }
            ...
            <div className='p-[20px]'>
                <div className='flex mb-3'>
                    ...
                    <select
                        ...
                        value={deptValue}
                        //value={ dept.find((d:Department) => d.department_name === subpayload.department?.department_name)?.id}
                        onChange={(e) => {
                            setSubpayload(prev => ({
