...
class KPDetector(nn.Module):
    ...
    def gaussian2kp(self, heatmap):
        shape = heatmap.shape
        # 1. 建立 grid 並確保設備一致
        grid = make_coordinate_grid(shape[2:], heatmap)
        grid = grid.unsqueeze(0).unsqueeze(0) # 擴展到 (1, 1, D, H, W, 3)
        
        if grid.device != heatmap.device:
            grid = grid.to(heatmap.device)

        # 2. 計算加權平均 (Soft-argmax 邏輯)
        # heatmap: (B, N, D, H, W)
        # grid: (1, 1, D, H, W, 3)
        # 我們需要對 D, H, W 維度進行加總
        
        # 增加 heatmap 的維度以匹配 grid 的最後一維 (3)
        heatmap = heatmap.unsqueeze(-1) # (B, N, D, H, W, 1)
        
        # 相乘並在空間維度上求和
        # (B, N, D, H, W, 3) -> (B, N, 3)
        value = (heatmap * grid).sum(dim=(2, 3, 4))
        
        # 確保輸出只有 3 個維度 (Batch, Keypoints, 3)
        # 有時候多餘的 singleton dimensions 會導致 "too many indices"
        return {'value': value.view(shape[0], shape[1], 3)}
