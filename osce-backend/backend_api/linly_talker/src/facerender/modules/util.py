...
def kp2gaussian(kp, spatial_size, kp_variance):
    """
    將關鍵點轉換為高斯分佈表示，並確保在 macOS MPS 上設備一致。
    """
    mean = kp['value']
    device = mean.device  # 獲取關鍵點所在的設備 (mps:0)
    dtype = mean.dtype

    # 1. 建立 grid 並強制同步設備
    coordinate_grid = make_coordinate_grid(spatial_size, mean)
    coordinate_grid = coordinate_grid.to(device=device, dtype=dtype)

    # 2. 處理維度擴展
    number_of_leading_dimensions = len(mean.shape) - 1
    # 建立與 mean 前綴維度匹配的 shape
    grid_shape = (1,) * number_of_leading_dimensions + coordinate_grid.shape
    coordinate_grid = coordinate_grid.view(*grid_shape)
    
    # 3. 進行維度重複 (Broadcasting)
    repeats = mean.shape[:number_of_leading_dimensions] + (1, 1, 1, 1)
    coordinate_grid = coordinate_grid.repeat(*repeats)

    # 4. 預處理 mean 的形狀以便相減
    # 將 (B, N, 3) 轉為 (B, N, 1, 1, 1, 3) 以便與 grid 做廣播運算
    mean_shape = mean.shape[:number_of_leading_dimensions] + (1, 1, 1, 3)
...
def make_coordinate_grid(spatial_size, type):
    ...
    # 關鍵修正：從傳入的 type 中提取 device 和 dtype
    # 在 Mac MPS 環境下，type 通常是一個具備 .device 和 .dtype 屬性的 Tensor
    device = type.device if hasattr(type, 'device') else 'cpu'
    target_dtype = type.dtype if hasattr(type, 'dtype') else torch.float32

    # 使用 .to() 確保新建立的 Tensor 直接位於正確的設備上
    x = torch.arange(w).to(device=device, dtype=target_dtype)
    y = torch.arange(h).to(device=device, dtype=target_dtype)
    z = torch.arange(d).to(device=device, dtype=target_dtype)
